...
const getRowColor = (slot) => {
  if (slot.isOverBudget) return 'bg-red-200 dark:bg-red-700/60';
  if (!slot.employeeName && slot.isAutoGenerated) return 'bg-green-200 dark:bg-green-700/60';
  if (slot.employeeName) return 'bg-blue-200 dark:bg-blue-700/60';
  return 'bg-gray-100 dark:bg-slate-800/40';
};
            ...
            <TableBody>
              {dailyScheduleData.map((slot, index) => (
                <TableRow 
                  ...
                  className={cn(getRowColor(slot), 'print:text-black')}
                >
                  <TableCell className={cn('font-medium')}>{slot.role}</TableCell>
