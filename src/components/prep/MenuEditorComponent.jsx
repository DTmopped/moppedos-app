...
const MenuEditorComponent = ({
  ...
  return (
    <>
      {/* ✅ Visual Debug Box */}
      <div className="mb-4 p-2 bg-yellow-100 border border-yellow-400 text-yellow-800 rounded text-center font-semibold">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {sections.map((section) => (
          <div
            key={section}
            className="bg-white border border-slate-200 rounded-xl shadow p-6"
          >
                ...
                {(menu[section] || []).map((item) => (
                  ...
                  >
                    <span className="text-slate-800">
                      {item.name}{" "}
                      {item.unit === "oz" &&
                        `– ${item.perGuestOz || item.value} oz`}
                      {item.unit === "each" &&
                        `– ${item.each || item.value} each`}
                      {item.unit === "lb" &&
                        `– ${item.each || item.value} lb`}
                    </span>
                    <button
