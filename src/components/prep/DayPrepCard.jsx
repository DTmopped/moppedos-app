...
const DayPrepCard = ({ dayData, onPrepTaskChange }) => {
  ...
  const getFallbackDisplayText = (item) => {
    let displayText = "General Task"; // Default for items without specific quantity/unit directives
    if (item && item.originalString && item.originalString.includes(':')) {
      const parts = item.originalString.split(':', 2);
      // Use the part after the colon if it exists and is not empty, otherwise use the part before if it's different from item.name
      const detail = parts.length > 1 ? parts[1].trim() : "";
      if (detail) {
        displayText = detail;
      } else if (parts[0].trim() && parts[0].trim().toLowerCase() !== item.name.toLowerCase()) {
                  ...
                  <Table className="min-w-full">
                    ...
                    <TableBody>
                      {shiftInfo.prepItems.map((item) => {
                        ...
                        let displayQuantity;
                        if (item && (typeof item.quantity === 'string' || typeof item.quantity === 'number') && String(item.quantity).trim() !== "") {
                        ...
                        let displayUnit;
                         if (item && typeof item.unit === 'string' && item.unit.trim() !== "") {
                            displayUnit = item.unit;
                        } else if (item && typeof item.unit === 'number') { // Handle case where unit might be a number
                            displayUnit = String(item.unit);
                        } else {
                            // Show fallback only if quantity is also N/A or effectively General Task
                            displayUnit = (displayQuantity !== "N/A" && displayQuantity !== "General Task") ? "" : fallbackText;
                        }
                        
                        // If quantity is N/A but unit has become fallbackText (and isn't 'General Task'), clear quantity to not show "N/A"
                        ...
                        return (
                          <TableRow key={item.id} className="hover:bg-muted/20 dark:hover:bg-slate-800/30 transition-colors">
                            ...
                            <TableCell className={cn(
                                ...
                                (displayQuantity === "N/A" || displayQuantity === "") ? "text-muted-foreground italic" : "text-primary font-semibold"
                              )}>
                              {displayQuantity}
                            </TableCell>
