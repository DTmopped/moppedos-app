...
// Grab target percentages from localStorage
      ...
      <table className="min-w-full text-sm text-left border border-slate-300">
        ...
        <tbody>
          {combinedData.map((entry, index) => {
            ...
            const dollarVar =
              hasActuals && typeof forecastSales === "number"
                ? actualSales - forecastSales
                : null;

            const pctVar =
              hasActuals && forecastSales
                ? ((actualSales - forecastSales) / forecastSales) * 100
                : null;

            const foodClass = hasActuals
              ? cn(
                  "font-semibold",
                  foodPct > foodTarget
            ...
            return (
              <tr key={index} className="border-t border-slate-200">
                ...
                <td className="px-3 py-2">
                  {hasActuals ? formatCurrency(actualSales) : "N/A"}
                </td>
                <td className={`px-3 py-2 ${varianceClass(dollarVar)}`}>
                  {formatCurrency(dollarVar)}
                </td>
                <td className={`px-3 py-2 ${varianceClass(pctVar)}`}>
                  {formatPercent(pctVar)}
                </td>
                <td className="px-3 py-2">
                  {hasActuals ? (
                    <span className={foodClass}>
                ...
                <td className="px-3 py-2">
                  {hasActuals ? (
                    <span className={bevClass}>
                      {(bevPct * 100).toFixed(1)}%
                    </span>
                  ) : (
                    "N/A"
                  )}
                ...
                <td className="px-3 py-2">
                  {hasActuals ? (
                    <span className={laborClass}>
                      {(laborPct * 100).toFixed(1)}%
                    </span>
                  ) : (
                    "N/A"
                  )}
