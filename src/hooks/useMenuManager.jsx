...
const initialMenuData = { /* same as before */ };

const MenuEditorComponentRaw = React.memo(({ sectionTitleColor, menu, editorsVisibility, newItemForms, toggleEditor, handleNewItemChange, addMenuItem, removeMenuItem }) => (
  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12 px-2 sm:px-4">
    {Object.keys(menu).map(section => (
      <Card key={`editor-${section}`} className="shadow-lg bg-slate-800/70 border-slate-700 backdrop-blur-sm">
        <CardHeader>
...
export const useMenuManager = (localStorageKey) => {
  ...
  const toggleEditor = useCallback((section) => {
    setEditorsVisibility(prev => ({ ...prev, [section]: !prev[section] }));
  }, []);

  const handleNewItemChange = useCallback((section, field, value) => {
  ...
  const addMenuItem = useCallback((section) => {
  ...
  const removeMenuItem = useCallback((section, name) => {
    setMenu(prev => ({
      ...prev,
