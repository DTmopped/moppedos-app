 ...
 * Returns items grouped by category (ordered by category_rank then item_name).
 */
export function useOrderGuide({ locationId, category = null, includeInactive = true } = {}) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // ✅ enforce the intended render order
      query = query
  ...
  // Group into { [category]: [items...] } preserving insertion order from the ordered rows
  const itemsByCategory = useMemo(() => {
    ...
    for (const r of rows) {
      ...
      grouped[cat].push({
        ...
        itemId : r.item_id ?? null,       // legacy compatibility
        name   : r.item_name || '',
        unit   : r.unit ?? '',
        actual,
        forecast,
