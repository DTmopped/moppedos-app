  ...
  const analyzeData = useCallback(() => {
    ...
    try {
      ...
      if (isPerformanceLogFormat) {
        ...
        if (parsedEntries.length === 0 || parsedEntries.some(e => e === null || Object.keys(e).length === 0)) {
          setError("No valid performance log entries found or some entries are invalid. Check format.");
          toast({ title: "Parsing Error", description: "No valid performance log entries found or some entries are invalid. Check format.", variant: "destructive", action: <AlertTriangle className="text-red-500"/> });
        } else {
        ...
        if (parsedEntry && Object.keys(parsedEntry).length > 0) {
            setAnalysisResult({ type: "posActuals", data: [parsedEntry] });
        } else {
            setError("Failed to parse POS Actuals data. Please check the format.");
            toast({ title: "Parsing Error", description: "Failed to parse POS Actuals data. Check format.", variant: "destructive", action: <AlertTriangle className="text-red-500"/> });
        }
      }
    } catch (e) {
