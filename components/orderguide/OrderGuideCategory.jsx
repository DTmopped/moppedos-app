...
import { cn } from '../../src/lib/utils'; For conditional class names
              ...
              {items.map(([name, forecast, unit, actual, variance], index) => {
                ...
                return (
                  <TableRow key={`${categoryTitle}-${name}-${index}`} className={cn("hover:bg-slate-800/30 transition-colors duration-150", rowStatusClass)}>
                    <TableCell className="font-medium px-3 py-2.5">{name}</TableCell>
                    <TableCell className="text-right px-3 py-2.5">{typeof forecast === 'number' ? forecast.toLocaleString() : forecast}</TableCell>
                    <TableCell className="text-right px-3 py-2.5">{typeof actual === 'number' ? actual.toLocaleString() : (actual || "-")}</TableCell>
                    <TableCell className="text-right px-3 py-2.5">{variance}</TableCell>
                    <TableCell className="px-3 py-2.5">{unit}</TableCell>
                    <TableCell className="text-center no-print px-3 py-2.5">
